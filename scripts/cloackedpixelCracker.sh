...
check_result_file() {
    ...
    if [ ! "`file $RESULT_FILE`" = "$RESULT_FILE: data" ] && [ $SIZE -ge 1 ]; then
        echo ""
        echo "Found something interesting using password: $PASS (cloackedpixel extract $in_file $out_file $PASS)"
...
echo "Warning!! cloackedpixel is VERY SLOW..."
#Main
while read pass; do
    if (( verbose > 0 ));then 
        echo -en "$pass\r"
    fi
    cloackedpixel extract $in_file $out_file "$pass"  > /dev/null 2>&1
